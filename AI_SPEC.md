...
2. **累積報酬率%（value / 累積投入）**（隱私模式）  
   - **定義**：以累積投入現金作為基準，顯示到當日的報酬率：  
     ...
     \]
     其中 \(cumulativeCash_t=\sum_{d \le t}{dayCashTotal_d}\)。  
   - **例子**：若截至某日累積投入 \(cumulativeCash=20{,}000\)，當日市值 \(value=18{,}500\)，則  
     \[
     (18{,}500/20{,}000-1)\times 100\%=-7.5\%
     \]
...
3. **超額績效%（portfolio / vti）**（隱私模式）  
   - **定義**：顯示 portfolio 相對 VTI 的超額幅度：  
     ...
     \]
   - **例子**：若某日 `portfolioValue=21,000`、`vtiValue=20,000`，則  
     \[
     (21{,}000/20{,}000-1)\times 100\%=+5\%
     \]
   - **圖表呈現**：可用一條「超額%」曲線，並加一條 0% baseline。  
   - **隱私規則**：同上（不顯示交易金額）。

### 8.2 ViewMode：全部期間 / 年度視圖

圖表提供兩種時間範圍切換（ViewMode）：

1. **全部期間（trend）**  
   - **顯示內容**：從系統最早可查日期起，所有 BUY 事件的累積投資表現（含跨年度持倉）。  
   - **用途**：查看整體投資組合相對 VTI 的長期表現。  
   - **摘要**：不顯示年度摘要文字（圖表下方無投資組合/VTI 數值）。

2. **年度視圖（年份數字）**  
   - **顯示內容**：僅該年度新增投入（該年 BUY 事件）的表現；**不含**前期累積持倉。  
   - **用途**：比較「若當年度新增的錢全部買 VTI vs 原策略」的報酬差異。  
   - **摘要**：圖表下方顯示該年度的投資組合與 VTI 數值（依 valueMode 顯示金額/報酬率%/超額%）。  
   - **按鈕**：從 BUY 事件中擷取唯一年份並排序，每個年份一個按鈕。

- **實作**：`getYearsFromEvents(events)` 從 BUY 事件的 `isoDateET` 擷取年份、去重、升冪排序；`computeYearlyReturns(year)` 篩選該年 BUY 事件並重新計算 portfolio / VTI 序列。
